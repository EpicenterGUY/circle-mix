...
for(const forbidden of ['SCREEN_ORIENTATION_SENSOR_LANDSCAPE','requestedOrientation ='])if(activity.includes(forbidden))throw new Error(`MainActivity must not force startup orientation: ${forbidden}`);
for(const needle of ['android:appCategory="game"','android:resizeableActivity="true"','android:screenOrientation="unspecified"','smallestScreenSize','android.hardware.touchscreen','android.hardware.screen.landscape'])if(!manifest.includes(needle))throw new Error(`AndroidManifest is missing ${needle}`);
