...
if(!desktopRelease.includes('version:"0.9.36"')||!desktopRelease.includes('MOBILE LANDSCAPE + WINDOWS FIX')||!desktopRelease.includes('CircleMixChangelog'))throw new Error('desktop 0.9.36 release metadata is missing');
const releaseWindow={CircleMixVersion:{version:'0.9.35'},CircleMixChangelog:[{version:'0.9.35',title:'PC AIM INPUT V1'}]};
vm.runInNewContext(desktopRelease,{window:releaseWindow,Object},{filename:'desktop-dist/src/desktop-release.js'});
if(releaseWindow.CircleMixVersion?.version!=='0.9.36'||releaseWindow.CircleMixChangelog?.[0]?.version!=='0.9.36'||releaseWindow.CircleMixChangelog?.[0]?.title!=='MOBILE LANDSCAPE + WINDOWS FIX')throw new Error('desktop release metadata override did not expose 0.9.36');
