...
for(const file of ['pc-settings.css','src/pc-settings.js','src/ui.js','trackpad-tablet-area.css','src/trackpad-tablet-area.js','mobile-layout-v2.css','src/mobile-layout-v2.js'])if(!fs.existsSync(path.join(out,file)))throw new Error('mobile layout v2 or trackpad settings asset is missing: '+file);
...
if(!desktopRelease.includes(`version:"${desktopVersion}"`)||!desktopRelease.includes(`TRACKPAD INPUT ${desktopVersion}`)||!desktopRelease.includes('CircleMixChangelog'))throw new Error(`desktop ${desktopVersion} trackpad release metadata is missing`);
...
if(releaseWindow.CircleMixVersion?.version!==desktopVersion||releaseWindow.CircleMixChangelog?.[0]?.version!==desktopVersion||releaseWindow.CircleMixChangelog?.[0]?.title!==`TRACKPAD INPUT ${desktopVersion}`)throw new Error(`desktop release metadata override did not expose ${desktopVersion}`);
...
for(const needle of ['pc-settings.css','src/pc-settings.js','trackpad-tablet-area.css','src/trackpad-tablet-area.js','mobile-layout-v2.css','src/mobile-layout-v2.js'])if(!desktopBootstrap.includes(needle))throw new Error('desktop settings bootstrap is missing: '+needle);
