...
if(!desktopRelease.includes('version:"0.9.33"')||!desktopRelease.includes('PULSE & SWING VISUAL PASS')||!desktopRelease.includes('CircleMixChangelog'))throw new Error('desktop visual pass release metadata is missing');
const releaseWindow={CircleMixVersion:{version:'0.9.32'},CircleMixChangelog:[{version:'0.9.32',title:'PULSE TUTORIAL HOTFIX'}]};
vm.runInNewContext(desktopRelease,{window:releaseWindow,Object},{filename:'desktop-dist/src/desktop-release.js'});
if(releaseWindow.CircleMixVersion?.version!=='0.9.33'||releaseWindow.CircleMixChangelog?.[0]?.version!=='0.9.33'||releaseWindow.CircleMixChangelog?.[0]?.title!=='PULSE & SWING VISUAL PASS')throw new Error('desktop release metadata override did not expose the visual pass');
