...
if(!desktopRelease.includes('version:"0.9.35"')||!desktopRelease.includes('PC AIM INPUT V1')||!desktopRelease.includes('CircleMixChangelog'))throw new Error('desktop aim release metadata is missing');
const releaseWindow={CircleMixVersion:{version:'0.9.34'},CircleMixChangelog:[{version:'0.9.34',title:'WINDOWS UPDATER BOOTSTRAP'}]};
vm.runInNewContext(desktopRelease,{window:releaseWindow,Object},{filename:'desktop-dist/src/desktop-release.js'});
if(releaseWindow.CircleMixVersion?.version!=='0.9.35'||releaseWindow.CircleMixChangelog?.[0]?.version!=='0.9.35'||releaseWindow.CircleMixChangelog?.[0]?.title!=='PC AIM INPUT V1')throw new Error('desktop release metadata override did not expose PC aim v1');
