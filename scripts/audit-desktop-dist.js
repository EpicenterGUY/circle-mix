...
for(const file of ['pc-settings.css','src/pc-settings.js','mobile-layout-v2.css','src/mobile-layout-v2.js'])if(!fs.existsSync(path.join(out,file)))throw new Error('mobile layout v2 asset is missing: '+file);
...
if(!desktopRelease.includes('version:"0.9.41"')||!desktopRelease.includes('UNIFIED SETTINGS & MOBILE LAYOUT V2')||!desktopRelease.includes('CircleMixChangelog'))throw new Error('desktop 0.9.41 release metadata is missing');
...
if(releaseWindow.CircleMixVersion?.version!=='0.9.41'||releaseWindow.CircleMixChangelog?.[0]?.version!=='0.9.41'||releaseWindow.CircleMixChangelog?.[0]?.title!=='UNIFIED SETTINGS & MOBILE LAYOUT V2')throw new Error('desktop release metadata override did not expose 0.9.41');
...
const seenWindow={CircleMixVersion:{version:'0.9.40'},CircleMixChangelog:[],localStorage:{getItem(){return '0.9.41';}}};
retryClickCount=0;
