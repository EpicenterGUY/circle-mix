...
for(const file of ['pc-settings.css','src/pc-settings.js','src/ui.js','trackpad-tablet-area.css','src/trackpad-tablet-area.js','mobile-layout-v2.css','src/mobile-layout-v2.js','song-select-fixes.css'])if(!fs.existsSync(path.join(out,file)))throw new Error('mobile layout, song select, or trackpad settings asset is missing: '+file);
...
if(!desktopRelease.includes(`version:"${desktopVersion}"`)||!desktopRelease.includes(`LIBRARY FIXES ${desktopVersion}`)||!desktopRelease.includes('CircleMixChangelog'))throw new Error(`desktop ${desktopVersion} library release metadata is missing`);
...
if(releaseWindow.CircleMixVersion?.version!==desktopVersion||releaseWindow.CircleMixChangelog?.[0]?.version!==desktopVersion||releaseWindow.CircleMixChangelog?.[0]?.title!==`LIBRARY FIXES ${desktopVersion}`)throw new Error(`desktop release metadata override did not expose ${desktopVersion}`);
...
for(const needle of ['sortLocalDifficultyOrder','numericDifficulty','SOURCE_LOCAL?sortLocalDifficultyOrder'])if(!songRecord.includes(needle))throw new Error(`desktop local difficulty sorting is missing: ${needle}`);
