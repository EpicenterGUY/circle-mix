...
assert(Power.calculatePower({stars:5,accuracyRatio:1,comboRatio:.99,missCount:0,totalNotes:100})<perfect);
...
for(const item of preview){
  assert.equal(item.power,Power.calculatePower({stars:7,accuracyRatio:item.accuracyRatio,comboRatio:1,missCount:0,totalNotes:1}));
}
