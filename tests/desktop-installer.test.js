...
assert.equal(tauri.version,'0.9.47','Windows installer must publish the trackpad input release');
...
assert.match(settingsPass,/TRACKPAD_DESKTOP_RELEASE_PATCH/,'final desktop pass must stamp the trackpad release metadata');
assert.match(settingsPass,/package\.json/,'desktop release version must derive from package metadata');
assert.match(settingsPass,/TRACKPAD INPUT/,'desktop changelog must announce the trackpad input release');
for(const asset of ['pc-settings.css','src/pc-settings.js','src/ui.js','mobile-layout-v2.css','src/mobile-layout-v2.js'])assert.ok(settingsPass.includes(asset),`desktop settings pass includes ${asset}`);
...
assert.match(audit,/mobile layout v2 or trackpad settings asset is missing/,'desktop audit must verify trackpad and mobile layout packaging');
