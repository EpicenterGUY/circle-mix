...
assert.equal(tauri.version,'0.9.49','Windows installer must publish the library scroll and sort release');
...
assert.match(settingsPass,/LIBRARY FIXES/,'desktop changelog must announce the library fixes release');
for(const asset of ['pc-settings.css','src/pc-settings.js','src/ui.js','trackpad-tablet-area.css','src/trackpad-tablet-area.js','mobile-layout-v2.css','src/mobile-layout-v2.js','song-select-fixes.css'])assert.ok(settingsPass.includes(asset),`desktop settings pass includes ${asset}`);
...
assert.match(audit,/mobile layout, song select, or trackpad settings asset is missing/,'desktop audit must verify shared UI packaging');
