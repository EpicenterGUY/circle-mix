  ...
  function installBrowserTestApi(){
    ...
    window.CircleMixTestApi={
      startDeterministicChart:()=>{
        ...
        browserTestClock=0; resetAimInput(-Math.PI/2); resize();
      ...
      advanceTestClock:(seconds,step=.02)=>{
        ...
        while(browserTestClock<target){ const delta=Math.min(dt,target-browserTestClock); browserTestClock+=delta; updateArm(delta); updateNotes(browserTestClock,delta); focusNote=currentFocusNote(browserTestClock); testFrameCount++; testRenderCount++; }
        return window.CircleMixTestApi.state();
      },
      testInput:{
