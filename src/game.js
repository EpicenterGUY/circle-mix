(() => {
  ...
  function generateAnimaNormalChart(){
    ...
    // NORMAL HARD: 초견 가능 + 구조 명확. 빡세지만 TECH보다 여유 있음.

    // 0~32 Intro: 박자 기준 잡기
    anchor(n,4,20,4,[0,4,2,6]);
    stair(n,24,[0,2,4,6,1,3,5,7],1.0);
    swing(n,32,7,"CW");

    // 32~64 First phrase: CUT 중심 + 짧은 슬라이드
    ...
    // 72~112 Build: 길게 끌고 전환
    hold(n,74,6,5);
    motif(n,82,[6,4,2,0,1,3,5,7],.9);
    slide(n,96,7,3,"CCW",4);
    scratch(n,103,3,6,"CW",3.5);
    stair(n,108,[6,7,0,1],.5);

    // 112~144 Highlight 1: 첫 고밀도, 하지만 짧게
    burst(n,116,[0,1,2,3,4,5,6,7,7,6,5,4],.38);
    slide(n,126,4,0,"CCW",3.5);
    cut(n,132,0);
    swing(n,134,0,"CW");
    burst(n,136,[0,2,4,6,1,3,5,7],.40);
    scratch(n,142,7,3,"CCW",3.2);

    // 144~176 Break: 확실히 쉬게 둠
    hold(n,150,3,5);
    anchor(n,158,174,4,[0,4,2,6]);

    // 180~224 Build 2: 조금 더 어려움
    motif(n,182,[1,3,5,7,0,2,4,6,6,4,2,0],.75);
    slide(n,196,0,5,"CW",4);
    scratch(n,203,5,1,"CCW",3.5);
    stair(n,210,[1,2,3,4,5,6,7,0],.5);
    swing(n,216,0,"CW");
    slide(n,218,0,4,"CW",3.5);

    // 224~280 Climax: NORMAL의 메인 고밀도
    hold(n,224,4,4);
    burst(n,230,[4,5,6,7,0,1,2,3,3,2,1,0],.35);
    slide(n,240,0,5,"CW",4);
    cut(n,246,5);
    scratch(n,249,5,1,"CCW",3.5);
    burst(n,256,[1,3,5,7,0,2,4,6,7,5,3,1],.38);
    slide(n,268,1,6,"CW",4);
    swing(n,274,6,"CCW");

    // 280~320 Final drive: 하이라이트 유지, 연타는 짧게
    burst(n,282,[6,7,0,1,2,3,4,5,5,4,3,2],.36);
    scratch(n,292,2,7,"CW",3.5);
    stair(n,298,[7,5,3,1,6,4,2,0],.45);
    slide(n,306,0,4,"CW",4);
    swing(n,313,4,"CW");
    burst(n,316,[4,6,0,2,5,7,1,3],.40);

    // 320~342 Ending: 마무리는 명확하게
    hold(n,322,3,4);
    anchor(n,328,334,2,[0,2,4,6]);
    scratch(n,335,6,2,"CCW",3);
    swing(n,339,2,"CW");
    cut(n,341,4);
  ...
  function generateAnimaTechChart(){
    ...
    // TECH HUMAN HARD:
    // 사람 가능 기준으로 짧은 16분 러시 + 방향전환 + 경로 노트.
    // 긴 고속연타는 피하고, 하이라이트마다 짧게 몰아침.

    // 0~32 Intro: 너무 빽빽하게 시작하지 않음
    anchor(n,4,16,3,[0,4,2,6,1]);
    stair(n,22,[0,2,4,6,1,3,5,7],.75);
    swing(n,31,7,"CW");

    // 32~64 First phrase: 읽히는 중간밀도
    motif(n,36,[0,2,4,6,7,5,3,1,0,1,2,3],.62);
    slide(n,46,3,7,"CW",3.5);
    cut(n,52,7);
    swing(n,53,7,"CCW");
    burst(n,56,[7,6,5,4,3,2,1,0],.38);
    anchor(n,62,70,3,[0,4,2]);

    // 72~112 Build-up: 홀드와 경로로 긴장감
    hold(n,74,6,5);
    motif(n,82,[6,4,2,0,1,3,5,7,7,5,3,1],.62);
    slide(n,94,1,5,"CW",4);
    scratch(n,101,5,0,"CCW",3.5);
    burst(n,107,[0,1,2,3,4,5,6,7],.36);

    // 112~144 Highlight 1: 첫 빡센 구간
    burst(n,116,[0,1,2,3,4,5,6,7,7,6,5,4,3,2,1,0],.30);
    slide(n,126,0,4,"CW",3.6);
    swing(n,132,4,"CW");
    burst(n,134,[4,6,0,2,5,7,1,3,4,5,6,7],.32);
    scratch(n,142,7,3,"CCW",3.2);

    // 144~176 Break: 일부러 손 풀게 함
    hold(n,148,3,5.5);
    anchor(n,156,172,3.5,[0,4,2,6]);
    slide(n,174,6,2,"CCW",4);

    // 180~224 Build 2: 하이라이트 예열
    motif(n,182,[2,4,6,0,1,3,5,7,7,5,3,1,0,2,4,6],.55);
    scratch(n,196,6,1,"CW",3.5);
    burst(n,202,[1,2,3,4,5,6,7,0,0,2,4,6,1,3,5,7],.31);
    swing(n,213,7,"CCW");
    slide(n,216,7,3,"CCW",4);

    // 224~280 Climax: 메인 하이라이트. 가장 리겜답게.
    hold(n,224,3,4);
    burst(n,230,[3,4,5,6,7,0,1,2,2,1,0,7,6,5,4,3],.28);
    slide(n,240,3,0,"CW",4,1);
    swing(n,246,0,"CW");
    scratch(n,249,0,4,"CCW",3.5);
    burst(n,256,[4,6,0,2,5,7,1,3,0,1,2,3,4,5,6,7],.28);
    slide(n,268,7,2,"CCW",4);
    swing(n,274,2,"CCW");
    stair(n,276,[2,3,4,5,6,7,0,1],.42);

    // 280~320 Final drive: 짧은 피크 러시. 빡세지만 지속시간 제한.
    burst(n,282,[0,1,2,3,4,5,6,7,7,6,5,4,3,2,1,0],.26);
    scratch(n,292,0,5,"CW",3.4);
    burst(n,298,[5,7,1,3,6,0,2,4,7,5,3,1,6,4,2,0],.27);
    slide(n,310,0,6,"CW",4);
    swing(n,316,6,"CW");
    burst(n,318,[6,7,0,1,2,3,4,5,5,3,1,7],.30);

    // 320~342 Ending: 마지막은 과도한 난사보다 확실한 마무리
    hold(n,322,5,4);
    motif(n,328,[5,3,1,7,0,2,4,6],.55);
    scratch(n,335,6,2,"CCW",3);
    swing(n,339,2,"CW");
    cut(n,341,4);
