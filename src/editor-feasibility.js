(() => {
  ...
  function decorateRows(){const severityClasses=['severity-yellow','severity-orange','severity-red'];document.querySelectorAll('.noteRow').forEach(row=>{const index=Number(row.dataset.i),severity=highestForNote(index),wanted=severity?`severity-${severity}`:'';for(const name of severityClasses)if(name!==wanted&&row.classList.contains(name))row.classList.remove(name);if(wanted&&!row.classList.contains(wanted))row.classList.add(wanted);let badge=row.querySelector('.issueBadge');if(!severity){badge?.remove();return;}if(!badge){badge=document.createElement('b');badge.className='issueBadge';row.querySelector('span')?.append(' ',badge);}const badgeClass=`severity-${severity}`;for(const name of severityClasses)if(name!==badgeClass&&badge.classList.contains(name))badge.classList.remove(name);if(!badge.classList.contains(badgeClass))badge.classList.add(badgeClass);if(badge.textContent!==severity.toUpperCase())badge.textContent=severity.toUpperCase();});}
  function renderTimelineMarkers(){const timeline=$('timeline'),canvas=$('timelineCanvas');if(!timeline||!canvas)return;timeline.querySelector('.feasibilityTimelineOverlay')?.remove();const project=state.project,result=state.result;if(!project||!result?.issues?.length){timeline.classList.remove('hasFeasibilityOverlay');return;}timeline.classList.add('hasFeasibilityOverlay');const overlay=document.createElement('div'),displayWidth=Math.max(1,canvas.clientWidth||canvas.getBoundingClientRect().width||canvas.width),scale=displayWidth/Math.max(1,canvas.width);overlay.className='feasibilityTimelineOverlay';overlay.style.width=`${displayWidth}px`;const bpm=Math.max(1,Number($('bpm')?.value)||120),offset=Number($('offset')?.value)||0,zoom=Number($('zoom')?.value)||140;Object.entries(result.highestByNote||{}).forEach(([key,severity])=>{const index=Number(key),note=project.notes?.[index];if(!note)return;const time=(Number(note.beat)||0)*60/bpm+offset,marker=document.createElement('button');marker.type='button';marker.className=`feasibilityTimelineMarker severity-${severity}`;marker.style.left=`${Math.max(0,time*zoom*scale-4)}px`;marker.title=`${severity.toUpperCase()} · NOTE #${index}`;marker.dataset.noteIndex=String(index);overlay.appendChild(marker);});timeline.appendChild(overlay);}
