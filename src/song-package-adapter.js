...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.CircleMixSongPackageAdapter=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){'use strict';
  ...
  function songRecordToFullPackageInput(record){if(!(record.audioBlob instanceof Blob)||!record.audioBlob.size) throw new Error('FULL export requires stored audio.');const metadata=record.audioMetadata||{};const duration=Number(metadata.duration||record.actualDuration||record.duration);if(!duration) throw new Error('FULL export requires decoded audio duration.');return {song:song(record),charts:chartEntries(record),audio:{blob:record.audioBlob,name:record.localAudioFileName||record.audioFileName||`audio.${metadata.extension||'mp3'}`},audioMetadata:{duration},...(record.jacketBlob?{jacket:{blob:record.jacketBlob,name:record.jacketName||'jacket.png'}}:{})};}
  return Object.freeze({chartEntries,songRecordToChartPackageInput,songRecordToFullPackageInput});
});
