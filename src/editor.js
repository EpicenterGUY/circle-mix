(() => {
  ...
  const angleToLane = deg => ((Math.round((((deg + 90) % 360 + 360) % 360) / 45) % 8));
  const laneToDeg = lane => -90 + lane * 45;
  ...
  function makeNote(angleDeg){
    ...
    const note = { type, beat, lane: angleToLane(angleDeg) };
    if(type === "fx") note.durationBeat = 4;
    if(type.startsWith("slide") || type.startsWith("trace")){ note.endLane = angleToLane(angleDeg + (type.endsWith("CCW") ? -90 : 90)); note.durationBeat = type.startsWith("trace") ? 1.5 : 4; }
    if(type.startsWith("scratch")){ note.endLane = angleToLane(angleDeg + (type.endsWith("CCW") ? -45 : 45)); note.durationBeat = .55; note.amount = Math.PI * .34; }
  ...
  function updateSelectedFromInputs(){ const n=selectedNote(); if(!n) return; pushHistory(); n.beat=Number($("noteBeat").value)||0; n.lane=angleToLane(Number($("noteAngle").value)||0); if($("noteEndAngle").value!=="") n.endLane=angleToLane(Number($("noteEndAngle").value)); n.direction=$("noteDirection").value; if($("noteDuration").value!=="") n.durationBeat=Number($("noteDuration").value)||0; renderAll(); autosave(); }
  function fillProps(){ const n=selectedNote(); $("noteBeat").value=n?.beat??""; $("noteAngle").value=n?laneToDeg(n.lane):""; $("noteEndAngle").value=n&&n.endLane!==undefined?laneToDeg(n.endLane):""; $("noteDirection").value=n?.direction || (n?.type?.endsWith("CCW")?"CCW":"CW"); $("noteDuration").value=n?.durationBeat??""; }
  function chartJson(){ const m=meta(); return { title:m.title, artist:m.artist, bpm:m.bpm, offset:m.offset, difficulty:m.difficulty, previewStart:m.previewStart, notes:state.notes.slice().sort((a,b)=>(a.beat??0)-(b.beat??0)) }; }
  ...
  function validate(){ const dur=audio.duration||Infinity, errors=[], warnings=[]; state.notes.forEach((n,i)=>{ const fam=n.type==="fx"?"hold":(n.type||"").replace(/CW|CCW/g,"").toLowerCase(); if(!NOTE_TYPES.includes(n.type)) errors.push(`#${i} unsupported type ${n.type}`); if((n.beat??0)<0 || noteTime(n)<0) errors.push(`#${i} negative time`); if(["fx","slideCW","slideCCW","trace","traceCW","traceCCW"].includes(n.type) && !(n.durationBeat>0)) errors.push(`#${i} ${fam} duration must be > 0`); if(noteTime(n)>dur) warnings.push(`#${i} outside song length`); if(n.lane===undefined || n.lane<0 || n.lane>7) errors.push(`#${i} invalid lane/angle`); if((n.type.startsWith("slide")||n.type.startsWith("trace")) && n.endLane===undefined) errors.push(`#${i} missing endLane`); }); $("validation").innerHTML=[...errors.map(e=>`<div class="err">ERROR ${e}</div>`),...warnings.map(w=>`<div class="warn">WARN ${w}</div>`)].join("") || "No errors."; return errors.length===0; }
  ...
  preview.addEventListener("click",e=>{ const r=preview.getBoundingClientRect(), x=(e.clientX-r.left)*preview.width/r.width-preview.width/2, y=(e.clientY-r.top)*preview.height/r.height-preview.height/2; addNote(normDeg(Math.atan2(y,x)*180/Math.PI)); });
