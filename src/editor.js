(() => {
  ...
  function makeNote(angleDeg){
    ...
    if(type.startsWith("slide") || type.startsWith("trace")){ note.endAngle = cleanDeg(angle + (type.endsWith("CCW") ? -90 : 90)); note.durationBeat = type.startsWith("trace") ? 1.5 : 4; }
  ...
  function updateSelectedFromInputs(){ const n=selectedNote(); if(!n) return; pushHistory(); n.beat=Number($("noteBeat").value)||0; n.angle=cleanDeg(Number($("noteAngle").value)||0); delete n.lane; if($("noteEndAngle").value!=="") { n.endAngle=cleanDeg(Number($("noteEndAngle").value)); delete n.endLane; } n.direction=$("noteDirection").value; if($("noteDuration").value!=="") n.durationBeat=Number($("noteDuration").value)||0; renderAll(); autosave(); }
  function fillProps(){ const n=selectedNote(); $("noteBeat").value=n?.beat??""; $("noteAngle").value=n?noteAngle(n):""; $("noteEndAngle").value=n?noteEndAngle(n):""; $("noteDirection").value=n?.direction || (n?.type?.endsWith("CCW")?"CCW":"CW"); $("noteDuration").value=n?.durationBeat??""; }
  ...
  function validate(){ const dur=audio.duration||Infinity, errors=[], warnings=[]; state.notes.forEach((n,i)=>{ const fam=n.type==="fx"?"hold":(n.type||"").replace(/CW|CCW/g,"").toLowerCase(); if(!NOTE_TYPES.includes(n.type)) errors.push(`#${i} unsupported type ${n.type}`); if((n.beat??0)<0 || noteTime(n)<0) errors.push(`#${i} negative time`); if(["fx","slideCW","slideCCW","trace","traceCW","traceCCW"].includes(n.type) && !(n.durationBeat>0)) errors.push(`#${i} ${fam} duration must be > 0`); if(noteTime(n)>dur) warnings.push(`#${i} outside song length`); if(n.angle!==undefined && !Number.isFinite(Number(n.angle))) errors.push(`#${i} invalid angle`); if(n.angle===undefined && (n.lane===undefined || n.lane<0 || n.lane>7)) errors.push(`#${i} invalid lane/angle`); if((n.type.startsWith("slide")||n.type.startsWith("trace")) && n.endAngle===undefined && n.endLane===undefined) errors.push(`#${i} missing endAngle`); const end=noteEndAngle(n); if((n.type.startsWith("slide")||n.type.startsWith("trace")) && end!=="" && shortestAngleDifference(noteAngle(n),end)<6) warnings.push(`#${i} path start/end angles are very close`); }); for(let i=0;i<state.notes.length;i++) for(let j=i+1;j<state.notes.length;j++){ if(Math.abs((state.notes[i].beat||0)-(state.notes[j].beat||0))<0.001 && shortestAngleDifference(noteAngle(state.notes[i]),noteAngle(state.notes[j]))<8) warnings.push(`#${i}/#${j} same-time angles overlap visually`); } $("validation").innerHTML=[...errors.map(e=>`<div class="err">ERROR ${e}</div>`),...warnings.map(w=>`<div class="warn">WARN ${w}</div>`)].join("") || "No errors."; return errors.length===0; }
