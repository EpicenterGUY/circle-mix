(() => {
  ...
  const TAU = Math.PI * 2;
  const NOTE_TYPES = ["cut","fx","slideCW","slideCCW","trace","traceCW","traceCCW","swingCW","swingCCW","scratchCW","scratchCCW","pulse"];
  ...
  function makeNote(angleDeg){
    ...
    const note = type==="pulse" ? {type,beat} : { type, beat, angle };
  ...
  function updateSelectedFromInputs(){ const n=selectedNote(); if(!n) return; pushHistory(); n.beat=Number($("noteBeat").value)||0; if(n.type==="pulse"){ delete n.angle; delete n.lane; delete n.endAngle; delete n.endLane; delete n.direction; delete n.durationBeat; delete n.sweepAngle; delete n.turns; } else { n.angle=cleanDeg(Number($("noteAngle").value)||0); delete n.lane; if($("noteEndAngle").value!=="") { n.endAngle=cleanDeg(Number($("noteEndAngle").value)); delete n.endLane; } n.direction=$("noteDirection").value; if($("noteDuration").value!=="") n.durationBeat=Number($("noteDuration").value)||0; if($("noteSweepAngle")?.value!=="") n.sweepAngle=Number($("noteSweepAngle").value)||0; else delete n.sweepAngle; if($("noteTurns")?.value!=="") n.turns=Number($("noteTurns").value)||0; else delete n.turns; } renderAll(); autosave(); }
  function fillProps(){ const n=selectedNote(), pulse=n?.type==="pulse"; $("noteBeat").value=n?.beat??""; $("noteAngle").value=pulse?"":(n?noteAngle(n):""); $("noteEndAngle").value=pulse?"":(n?noteEndAngle(n):""); $("noteDirection").value=n?.direction || (n?.type?.endsWith("CCW")?"CCW":"CW"); $("noteDuration").value=pulse?"":(n?.durationBeat??""); if($("noteSweepAngle")) $("noteSweepAngle").value=pulse?"":(n?.sweepAngle??""); if($("noteTurns")) $("noteTurns").value=pulse?"":(n?.turns??""); for(const id of ["noteAngle","noteEndAngle","noteDirection","noteDuration","noteSweepAngle","noteTurns"]) if($(id)) $(id).disabled=!!pulse; }
  ...
  function fullRights(){ return new Promise(resolve=>{const d=document.createElement("dialog");d.innerHTML=`<form method="dialog" style="max-width:560px"><h2>FULL .CMIX audio redistribution</h2><p>FULL .cmix에는 음원 파일이 포함됩니다. 직접 제작한 음원이거나, 음원을 재배포할 권한 또는 라이선스를 가진 경우에만 내보내세요. 권한이 없다면 CHART .cmix를 사용하세요.</p><p>CIRCLE MIX는 권리 보유 여부를 검증하지 않습니다.</p><label><input required type="radio" name="rights" value="own"> 내가 직접 제작했거나 권리를 보유한 음원</label><br><label><input type="radio" name="rights" value="permission"> 권리자로부터 재배포 허가를 받은 음원</label><br><label><input type="radio" name="rights" value="license"> 퍼블릭 도메인 또는 재배포를 허용하는 라이선스 음원</label><p><button value="cancel">CANCEL / USE CHART .CMIX</button> <button id="fullRightsContinue" value="ok" disabled>CONTINUE</button></p></form>`;document.body.append(d);const radios=d.querySelectorAll('input');radios.forEach(x=>x.onchange=()=>d.querySelector('#fullRightsContinue').disabled=false);d.onclose=()=>{const ok=d.returnValue==='ok';d.remove();resolve(ok)};d.showModal()}) }
  async function exportFull(allDifficulties=false){const button=$(allDifficulties?'exportAllFullCmixBtn':'exportFullCmixBtn'), exporter=window.CircleMixCmixExporter, audioTools=window.CircleMixCmixAudio,m=meta();if(!exporter||!audioTools||!state.audioFile){$("validation").innerHTML='<div class="err">EXPORT ERROR: select a valid local audio file first.</div>';return} button.disabled=true;const old=button.textContent;button.textContent='CHECKING AUDIO...';try{const name=state.audioFile.name||state.localRecord?.localAudioFileName||`${m.songId}.wav`,file=state.audioFile.name?state.audioFile:new File([state.audioFile],name,{type:state.audioFile.type});const duration=await audioTools.metadata(file);const descriptor={id:$("cmixChartId").value,name:$("cmixChartName").value,level:Number($("cmixLevel").value),style:$("cmixStyle").value.trim(),author:$("cmixAuthor").value.trim()};let charts=[{descriptor,chart:chartJson()}];if(allDifficulties){if(!state.localRecord)throw new Error('No LOCAL SONGS record is loaded.');charts=Object.entries(state.localRecord.charts||{}).map(([key,chart])=>{const d=state.localRecord.difficulties?.[key]||{};return {descriptor:{id:d.id||key,name:d.name||d.label||key,level:Number(d.level||Math.max(1,Math.round(d.stars||10))),style:d.style||'CUSTOM',author:d.author||''},chart}})}if(!await fullRights())return;const result=await exporter.createFullPackage({song:{id:m.songId,title:m.title,artist:m.artist,bpm:m.bpm,offset:m.offset,packageVersion:Number($("cmixPackageVersion").value),preview:{startSeconds:Math.max(0,m.previewStart),durationSeconds:Math.min(30,duration)}},audio:{blob:file,name},audioMetadata:{duration},charts,jacket:$("cmixJacket").checked&&state.jacketFile?{blob:state.jacketFile,name:state.jacketFile.name}:null,onProgress:x=>button.textContent=String(x)});const summary=`FULL · ${result.manifest.id}\n${m.title} / ${m.artist}\nAUDIO: ${result.audioMetadata.format} · ${duration.toFixed(3)}s · ${result.audioMetadata.blob.size} bytes\nCHARTS: ${result.manifest.charts.map(x=>`${x.id} ${x.name} Lv.${x.level}`).join(', ')}\nJACKET: ${result.manifest.jacket||'not included'}\nPACKAGE: ${result.totalSize} bytes\nFILE: ${result.filename}`;if(!confirm(`FULL .CMIX validation passed. Download?\n\n${summary}`))return;const url=URL.createObjectURL(result.blob),a=document.createElement('a');a.href=url;a.download=result.filename;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);$("validation").innerHTML=`<div>FULL .CMIX exported: ${result.filename}</div>`}catch(error){$("validation").innerHTML=`<div class="err">EXPORT ERROR: ${error.message}</div>`}finally{button.disabled=false;button.textContent=old}}
  $("exportFullCmixBtn").onclick=()=>exportFull(false);
  $("exportAllFullCmixBtn").onclick=()=>exportFull(true);
