(function(root,factory){
  ...
  function isPcEnvironment(win){
    ...
    try{
      ...
      if(query.get('pcSettings')==='1')return true;
      if(query.get('pcSettings')==='0')return false;
    }catch(_){ }
  ...
  function availableSetting(doc,setting){
    ...
    return !!doc.getElementById(setting.target);
  }
  ...
  function loadUiState(win){
    try{
      ...
      return {group:GROUPS.some(group=>group.id===parsed.group)?parsed.group:'play'};
  ...
  function mount(doc,win){
    ...
    overlay.innerHTML=`
      <div class="pcSettingsShell">
        <header class="pcSettingsHeader">
          <div><span class="pcSettingsEyebrow">CIRCLE MIX · PC</span><h2 id="pcSettingsTitle">SETTINGS</h2></div>
        ...
        <div class="pcSettingsLayout">
          ...
          <main id="pcSettingsContent" class="pcSettingsContent"></main>
        </div>
        <footer class="pcSettingsFooter"><p>변경 사항은 즉시 적용됩니다. 모바일 전용 설정은 PC에서 표시하지 않습니다.</p><button id="pcSettingsReset" type="button">기본값 복원</button></footer>
      </div>`;
    ...
    function buildControl(setting,card){
      ...
      if(setting.kind==='stepper'){
        const down=create('button','pcSettingStep','−');
        const value=create('output','pcSettingValue','—');
        const up=create('button','pcSettingStep','＋');
        ...
        const link=create('a','pcSettingAction',setting.actionLabel||'열기');
        link.href=setting.href;
        box.append(link);
        card._value=link;
      }else{
        const action=create('button','pcSettingAction',setting.actionLabel||'—');
        action.type='button';
        action.addEventListener('click',()=>{if(setting.transient)close();triggerOriginal(doc,setting.target);scheduleRefresh();});
        box.append(action);
        card._value=action;
      }
      return box;
    ...
    for(const group of GROUPS){
      const button=create('button','pcSettingsNavButton');
      button.type='button';
      ...
      const section=create('section','pcSettingsSection');
    ...
    function currentDisplay(setting){
      ...
      return readNodeValue(doc,setting.target)||setting.actionLabel||'열기';
    }
    ...
    function refreshCard(card){
      ...
      const original=setting.target?doc.getElementById(setting.target):null;
      const unavailable=setting.kind!=='link'&&!availableSetting(doc,setting);
    ...
    function applyFilter(){
      ...
      for(const [groupId,section] of sections){
        ...
        section.hidden=query?visible===0:!active;
        total+=visible;
      }
      content.classList.toggle('isSearching',!!query);
      overlay.classList.toggle('hasNoResults',!!query&&total===0);
      let empty=content.querySelector('.pcSettingsEmpty');
      if(query&&total===0){
        if(!empty){empty=create('div','pcSettingsEmpty','일치하는 설정이 없습니다.');content.append(empty);}
      }else empty?.remove();
    }
    ...
    overlay.querySelector('#pcSettingsReset').addEventListener('click',()=>{
      if(!win.confirm('PC 설정을 기본값으로 복원하고 화면을 새로고침할까요?'))return;
      try{
    ...
    if(win.MutationObserver){
      for(const setting of SETTINGS){
        const ids=setting.kind==='stepper'?[setting.down,setting.up,setting.value]:[setting.target];
        for(const id of ids){
