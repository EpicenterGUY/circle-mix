...
(() => {
  ...
  function validateTraceSweep(n,i,errors,warnings){
    ...
    if(hasSweep && Number(n.sweepAngle)!==0 && sign && Math.sign(Number(n.sweepAngle))!==sign) errors.push(`#${i} direction conflicts with sweepAngle`);
    if(hasTurns && Number(n.turns)!==0 && Number(n.turns)<0 && sign) errors.push(`#${i} direction conflicts with negative turns`);
    if(noteEndAngle(n)!==undefined && shortestAngleDifference(noteAngle(n),noteEndAngle(n))<6 && !hasSweep && !hasTurns) warnings.push(`#${i} trace start/end angles match; add sweepAngle or turns to distinguish 0° from full rotations`);
  }
